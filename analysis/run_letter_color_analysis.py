...
mask_dir = os.path.join(home_dir,'masks')                   # brain masks
...
if run_preprocessing:
    for s,subject in enumerate(mri_subjects):
        for session in sessions:
            ...
            preprocess.dicom2bids('convert2bids_letter-color_main_sub-{}.json'.format(subjects_group[s]))
            print('converting sub-{} convert2bids_letter-color_main_sub-{}.json'.format(subject,subjects_group[s]))
...
if run_first_level:
    for s,subject in enumerate(subjects):
        ...
        # first_level.concatenate_epi_data()           # concatenate EPI data
