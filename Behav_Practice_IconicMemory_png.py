...
if subject_ID:
    ...
    fixcross        = visual.ImageStim(win, image = 'cross.png', units='deg', pos = [0,y_shift_up], size=fixcross_size)
    ...
    stim_cue_black  = visual.ImageStim(win, image = 'asterisk_black.png', units='deg', size=cue_size)
    stim_cue_grey   = visual.ImageStim(win, image = 'asterisk_grey200.png', units='deg', size=cue_size)
    ...
    for t in range(len(BTRIALS)):
        ...
        for c in range(len(block_letter_list)):
            if c == 0: #Target position-letter
                #stim_cue_black.setHeight(cue_size)
                current_pos = misc.pol2cart(target_pos_pol,eccen,units='deg')
                stim_cue_black.setPos([current_pos[0],current_pos[1]+y_shift_up])
                #stim_cue.setColor(cue_target_color)
                stim_cue_black.draw()
            else: #Other positions-letters
                #stim_cue_grey.setHeight(cue_size)
                current_pos = misc.pol2cart(pos_list[c-1],eccen,units='deg')
                stim_cue_grey.setPos([current_pos[0],current_pos[1]+y_shift_up]) # change
