...
cwd = os.getcwd()
logfile_dir = os.path.join(cwd,'source','sub-{}'.format(subject_ID),'ses-{}'.format(session),'behav') 
...
RGBS = [] # save output
random_colors = [] # save starting color positions
L = '2'  # place holder 
...
class Test():
    ...
    def pick_a_color(self,):        
        ...
        random_color = (random.randint(0,255), random.randint(0,255), random.randint(0,255)) # start with random color position
        random_colors.append(random_color) # add to list for output
        self.RGB,self.HEX = askcolor(random_color, parent=None, title='Pick a color: {}'.format(L) )
...
for R in np.arange(REPS):
    ...
    for L in alphabet:      
        ...
        try:
            ...
            DFS['random_colors'] = [rc for rc in random_colors]
        except:            
            # clicked window away
            pass
...
plt.tight_layout()
fig.savefig(os.path.join(cwd,'LogFiles','sub-{}'.format(subject_ID),'ses-{}'.format(session),'behav','sub-{}_ses-{}_colors.pdf'.format(subject_ID,session)))
print('success: sub-{}_ses-{}_colors.pdf'.format(subject_ID,session))
